...
test('removes legacy login storage without removing chat history', () => {
  ...
  const entries = new Map([
    ['tixora.session', '{"accessToken":"legacy"}'],
    ['team-task-manager.session', '{"accessToken":"older-legacy"}'],
    ['tixora.chat.v1:example', '[]']
  ]);
  try {
    ...
    assert.equal(entries.has('team-task-manager.session'), false);
